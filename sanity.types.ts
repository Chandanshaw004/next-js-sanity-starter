...
export type AllSanitySchemaTypes = SanityImagePaletteSwatch | SanityImagePalette | SanityImageDimensions | SanityFileAsset | Geopoint | AllProductPosts | AllPosts | FormNewsletter | Faqs | LogoCloud1 | Cta1 | Timelines1 | TimelineRow | Carousel2 | Carousel1 | GridRow | GridPost | PricingCard | GridCard | SplitInfo | SplitInfoList | SplitImage | SplitCard | SplitCardsList | SplitContent | SplitRow | SectionHeader | Hero2 | Hero1 | SectionPadding | ButtonVariant | ColorVariant | Link | BlockContent | Testimonial | Faq | Product | Post | Subcategory | Category | Author | GlobalContent | Page | SanityImageCrop | SanityImageHotspot | SanityImageAsset | SanityAssetSourceData | SanityImageMetadata | Slug | Code;
...
// Query: *[_type == "category"]{  _id,  title,  subcategory[]->{    _id,    title  }}
...
// Variable: GLOBAL_CONTENT_QUERY
// Query: *[_type == "globalContent"][0] {  headerLogo,    headerNavItems[]{    label,    href,    target,    subMenu[]{      label,      href,      target,      subSubMenu[]{      label,      href,      target,    }     }  },  footerLogo,    footerDescription,  socialLinks[]{    platform,    url,    icon  },  footerLinks[]{    title,    links[]{      title,      href    }  }}
export type GLOBAL_CONTENT_QUERYResult = {
  headerLogo: {
    asset?: {
      _ref: string;
      _type: "reference";
...
// Query: *[_type == "product" && slug.current == $slug][0]{    title,    slug,    image{      ...,      asset->{        _id,        url,        mimeType,        metadata {          lqip,          dimensions {            width,            height          }        }      },      alt    },        description,    body[]{      ...,      _type == "image" => {        ...,        asset->{          _id,          url,          mimeType,          metadata {            lqip,            dimensions {              width,              height            }          }        }      }    },        category->{    _id,    title  },  subcategory->{    _id,    title  },    _createdAt,    _updatedAt,    meta_title,    meta_description,    noindex,    ogImage {      asset->{        _id,        url,        metadata {          dimensions {            width,            height          }        }      },    }}
...
// Query: *[_type == "product" && defined(slug)] | order(_createdAt desc){    title,    slug,    description,    category->{    _id,    title  },  subcategory->{    _id,    title  },    image{      asset->{        _id,        url,        mimeType,        metadata {          lqip,          dimensions {            width,            height          }        }      },      alt    },}

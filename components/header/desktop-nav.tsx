    ...
    <div className="hidden xl:flex items-center gap-7 text-primary">
      {navItems.map((navItem) => (
        <div key={navItem.label} className="relative group">
          ...
          {/* Submenu */}
          {navItem.subMenu && (
            <div className="absolute top-3 left-0 mt-2 hidden group-hover:block bg-white shadow-lg rounded-md">
              <ul className="py-2">
                {navItem.subMenu.map((subItem) => (
                  <li key={subItem.label} className="relative group/submenu">
                    ...
                    {/* Sub-Submenu */}
                    {subItem.subSubMenu && (
                      <div className="absolute top-0 left-full mt-0 ml-1 hidden group-hover/submenu:block bg-white shadow-lg rounded-md">
                        <ul className="py-2">
